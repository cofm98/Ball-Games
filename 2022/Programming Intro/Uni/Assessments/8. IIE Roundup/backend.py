...
class Happiness:
    ...
    def initialise (self, name : str, last_name : str, happy : bool, happy_city_services : int, happy_housing : int, happy_schooling : int, happy_police : int, happy_street_maintenance : int, happy_community_events : int):
        self.happy = happy
        self.name = name
...
def Get_Data (fileURL : str) -> list:
    ...
    while nextLine != "":
        unformatted = Split_Data(nextLine[:-1])
        ...
        while x < len(unformatted):
            try:
                ...
                x+=1
        formatted = Happiness()
        formatted.initialise(unformatted[0], unformatted[1], likes_the_town,
...
def Split_Data (to_array : str) -> list[str]:
    output = [""]
    n = 0
...
def Save_Data (fileURL : str, data : list) -> str:
    input = ""
    x = 0
...
def Check_File (fileURL : str) -> bool:
    output = True
...
def add_record (data : list) -> list:
    sys.stdout.write("First Name: ")
    f_name = sys.stdin.readline().strip()
    sys.stdout.write("Last  Name: ")
    l_name = sys.stdin.readline().strip()
    happy = confirm("Is " + f_name + " " + l_name + " happy?")
    sys.stdout.write("> Happiness Scores for Public Services [1->5]...\n")
    city_services = get_score(" - City Services", 1, 5)
    housing = get_score(" - Cost of Housing", 1, 5)
    schooling = get_score(" - Schooling", 1, 5)
    policing = get_score(" - Policing", 1, 5)
    street_maintenance = get_score(" - Street Maintenance", 1, 5)
    community_events = get_score(" - Community Events", 1, 5)
    new_record = Happiness()
    new_record.initialise(f_name, l_name, happy, city_services, housing, schooling, policing, street_maintenance, community_events)
...
def get_score (question : str, min : int, max : int) -> int:
    score = min - 1
...
def get_main_menu (current_file : str, modified : bool) -> str:
    output = "\n  ________________\n"
...
def get_input (menu_type : str = None) -> str:
    output = ""
...
def get_load_file () -> str:
    ...
    while input == False:
        ...
        output = get_input() + ".csv"
        input = confirm("Are you sure you would like to load "+output+"?")
    return output
