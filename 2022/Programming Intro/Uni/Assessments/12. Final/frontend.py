...
class FrontEndUI():
    def __init__(self, back_end):
        ...
        pass
    def show_ui(self):
        file = self.__backend.get_load_file()
        self.__backend.Check_File(file)
        data = self.__backend.Get_Data(file)
        option = ""
        modified = False
        ...
        while option != "Q":
            main_menu = self.__backend.get_main_menu(file, modified)
            sys.stdout.write(main_menu)
            option = self.__backend.get_input("main")
            ...
            if option == "A":
                data = self.__backend.add_record(data)
                modified = True
            ...
            # This process is talked about further within the description of the function definition statements.
                ...
                if self.__backend.confirm("Would you like to save?"):
                    self.__backend.Save_Data(file, data)
                    modified = False
                file = self.__backend.get_load_file()
                self.__backend.Check_File(file)
                data = self.__backend.Get_Data(file)
            ...
            # itself and possibly hides the data above.
                ...
                self.__backend.Show_Records(data)
            ...
            # asking for confirmation to saving the file.
                ...
                if modified:
                    if self.__backend.confirm("Would you like to save before quitting?"):
                        self.__backend.Save_Data(file, data)
